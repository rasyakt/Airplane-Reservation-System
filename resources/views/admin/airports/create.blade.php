...
@section('content')
    <div class="max-w-2xl">
        <h2 class="text-xl font-semibold mb-6">Add New Airport</h2>

        <form action="{{ route('admin.airports.store') }}" method="POST" class="bg-gray-800 p-6 rounded-lg">
            @csrf

            <div class="mb-4">
                <label class="block text-sm font-medium mb-2">IATA Airport Code</label>
                <input type="text" name="iata_airport_code" maxlength="3"
                    class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 uppercase"
                    value="{{ old('iata_airport_code') }}" required>
